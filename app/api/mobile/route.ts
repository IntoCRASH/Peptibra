...
export async function POST(request: Request) {
  ...
  try {
    if(action==="createProduct"||action==="updateProduct"){
      ...
      const profile={product_id:productId,description:str(d.description),photo_key:str(d.photoKey),calculation_id:num(d.calculationId)||null,unit_cost:num(d.unitCost),normal_price:num(d.normalPrice),bac_price:num(d.bacPrice),wholesale_mg_price:num(d.wholesaleMgPrice),wholesale_minimum:num(d.wholesaleMinimum,10)};
    ...
    if(action==="createInvoice"||action==="updateInvoice"){
      const items=Array.isArray(d.items)?d.items as Data[]:[];if(!items.length)return json({error:"Agrega productos"},400);const sellerId=num(d.sellerId),seller=await one("team",sellerId),partnerId=seller?.role==="Socio"?sellerId:num(seller?.partner_id)||null,location=partnerId?`SOCIO:${partnerId}`:`VENDEDOR:${sellerId}`;
      let invoiceId=num(d.id),number=str(d.number);if(invoiceId){const {data:oldItems,error}=await cloud.from("invoice_items").select("*").eq("invoice_id",invoiceId);if(error)throw error;for(const item of oldItems||[])await changeBalance(Number(item.product_id),location,Number(item.quantity));await cloud.from("invoice_items").delete().eq("invoice_id",invoiceId)}else{const {count}=await cloud.from("invoices").select("id",{count:"exact",head:true});number=`PTBR-F${now.slice(0,7).replace("-","")}-${String((count||0)+1).padStart(5,"0")}`}
      ...
      for(const item of prepared){const qty=num(item.quantity),effectiveRate=(1-(1-num(item.progressiveRate)/100)*(1-manualRate/100))*100,line=qty*num(item.unitPrice)*(1-effectiveRate/100);const {error}=await cloud.from("invoice_items").insert({invoice_id:invoiceId,product_id:num(item.productId),quantity:qty,unit_price:num(item.unitPrice),unit_cost:num(item.unitCost),discount_pct:effectiveRate,total:line});if(error)throw error;await changeBalance(num(item.productId),location,-qty)}return json({ok:true,id:invoiceId,number});
    }
    ...
    if(action==="applyPayment"){
      const id=num(d.invoiceId),inv=await one("invoices",id);if(!inv)return json({error:"Factura no encontrada"},404);const rate=Math.max(.0001,num(d.exchangeRate,1)),original=num(d.originalAmount),usd=str(d.currency)==="DOP"?original/rate:original,due=num(inv.balance),applied=Math.min(usd,due),excess=Math.max(0,usd-due),choice=str(d.excessAction);if(excess>.005&&!choice)return json({error:"Selecciona devolver o dejar el excedente en caja",excess},409);const paid=num(inv.paid)+applied,balance=Math.max(0,num(inv.total)-paid),status=balance<=.005?"Pagada":"Parcial";let {error}=await cloud.from("invoices").update({paid,balance,status,updated_at:now}).eq("id",id);if(error)throw error;({error}=await cloud.from("payments").insert({invoice_id:id,applied_usd:applied,original_amount:original,currency:str(d.currency)||"USD",exchange_rate:rate,excess_usd:choice==="Caja"?excess:0,excess_action:choice,method:str(d.method)||"Efectivo",created_at:now}));if(error)throw error;await cloud.from("cash_movements").insert({type:"Ingreso",category:"Cobro factura",amount:applied,invoice_id:id,notes:str(inv.number),created_at:now});if(choice==="Caja"&&excess>.005)await cloud.from("cash_movements").insert({type:"Ingreso",category:"Excedente de cliente",amount:excess,invoice_id:id,notes:str(inv.number),created_at:now});return json({ok:true,applied,excess,status});
    }
    ...
    if(action==="deleteInvoice"){
      const id=num(d.invoiceId),inv=await one("invoices",id);if(!inv)return json({error:"Factura no encontrada"},404);const seller=await one("team",num(inv.seller_id)),owner=seller?.role==="Socio"?num(inv.seller_id):num(seller?.partner_id)||null,location=owner?`SOCIO:${owner}`:`VENDEDOR:${num(inv.seller_id)}`,{data:items}=await cloud.from("invoice_items").select("*").eq("invoice_id",id);for(const item of items||[])await changeBalance(Number(item.product_id),location,Number(item.quantity));await cloud.from("cash_movements").delete().eq("invoice_id",id);await cloud.from("payments").delete().eq("invoice_id",id);await cloud.from("invoice_items").delete().eq("invoice_id",id);const {error}=await cloud.from("invoices").delete().eq("id",id);if(error)throw error;return json({ok:true});
    }
    if(action==="archive"){
    ...
    if(action==="cashMovement"){const {error}=await cloud.from("cash_movements").insert({type:str(d.type),category:str(d.category),amount:num(d.amount),partner_id:num(d.partnerId)||null,notes:str(d.notes),created_at:now});if(error)throw error;return json({ok:true});}
