...
export async function POST(request: Request) {
  ...
  try {
    ...
    if(action==="createInvoice"||action==="updateInvoice"){
      ...
      const subtotal=items.reduce((s,x)=>s+num(x.quantity)*num(x.unitPrice),0),discount=Math.max(0,num(d.discount)),total=Math.max(0,subtotal-discount),values={number,client_id:num(d.clientId),seller_id:sellerId,partner_id:partnerId,subtotal,discount,total,balance:total,paid:0,status:"Pendiente",notes:str(d.notes),updated_at:now};
      ...
      for(const item of items){const qty=Math.max(1,Math.trunc(num(item.quantity,1))),line=Math.max(0,qty*num(item.unitPrice)-num(item.discount));const {error}=await cloud.from("invoice_items").insert({invoice_id:invoiceId,product_id:num(item.productId),quantity:qty,unit_price:num(item.unitPrice),unit_cost:num(item.unitCost),discount_pct:num(item.discountPct),total:line});if(error)throw error;await changeBalance(num(item.productId),location,-qty)}return json({ok:true,id:invoiceId,number});
    }
