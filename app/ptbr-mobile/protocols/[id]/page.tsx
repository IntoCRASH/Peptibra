...
export default async function ProtocolPage({params}:{params:Promise<{id:string}>}){
  ...
  const {data:product}=await s.from("products").select("*").eq("id",protocol.product_id).maybeSingle();
  ...
  return <ProtocolPrint protocol={protocol} product={product||{}} safety={safety}/>;
}
